...
class Minion extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      editing: editing,
      minion: props.newMinion
        ? { name: '', title: '', salary: 0, weaknesses: '' }
        : props.minion,
    };
  }

  componentDidUpdate(prevProps) {
  if (prevProps.minion !== this.props.minion) {
    this.setState({ minion: this.props.minion });
  }
}
  ...
  render() {
    console.log('Minion props:', this.props.minion);
    console.log('Minion state:', this.state.minion);
    ...
    return (
      <div>
        <div id="single-minion-landing">
          <div className="minion-details">
            ...
            <div className="minion-description">
              { 
                this.state.editing
                ? <MinionEdit handleChange={this.handleChange} {...this.state.minion} />
        ...
        <div className="button back-button">
          <Link to="/minions">
            <img className="button" src="/img/arrow.svg" alt="Back to minions"/>
          </Link>
        </div>
      </div>
    )
