...
class Idea extends Component {
  ...
  render() {
    console.log('newIdea prop:', this.props.newIdea);
    console.log('editing state:', this.state.editing);
    console.log('idea state:', this.state.idea);


    const isValid = this.props.newIdea
      ? true
      : isMillionDollarIdea(this.state.idea.weeklyRevenue, this.state.idea.numWeeks);
    const buttonText = this.state.editing ? ( isValid ? 'Save' : 'Not A Valid $1000000 Idea!') : 'Edit';
    return (
      <div>
        <div id="single-idea-landing">
          { this.state.editing
            ? <IdeaEdit idea={this.state.idea} handleChange={this.handleChange} />
        ...
        <div className="button back-button">
          <Link to="/ideas">
            <img className="button" src="/img/arrow.svg" alt="Back to ideas"/>
          </Link>
        </div>
      </div>
    )
